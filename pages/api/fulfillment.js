...
export default async function handler(req, res) {
  ...
  console.log(body, headers.authorization);
  ...
  if (inputs[0].intent == "action.devices.EXECUTE") {
    ...
    console.debug("ExecuteResponse:", executeResponse);
    return res.status(200).json(executeResponse);
  }
  if (inputs[0].intent == "action.devices.DISCONNECT") {
    ...
    const disconnectResponse = {};
    console.debug("DisconnectResponse:", disconnectResponse);
    return res.status(200).json(disconnectResponse);
  }
}

// inputs: [ { intent: 'action.devices.SYNC' } ],
// requestId: '17910060430995818032'
